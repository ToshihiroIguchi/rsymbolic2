...
test_that("adaptive_parsimony_scaling=0 (default) preserves behaviour and recovers", {
  skip_on_cran()
  X <- matrix(seq(-3, 3, length.out = 20), ncol = 1)
  y <- X[, 1]^2 + 2 * X[, 1] + 1
  # Default path (scaling not passed) and an explicit scaling=0 must be identical:
  # scaling<=0 disables the frequency factor, so selection is byte-for-byte unchanged.
