...
fixed hyperparameter** (this is also true in PySR, where
`adaptive_parsimony_scaling` defaults to a fixed 1040.0). "Adaptive" refers to the
...
(anti-collapse). The numeric `scaling` is also not transferable from PySR's 1040
because our normalization scale differs; it is tuned by measurement below.
