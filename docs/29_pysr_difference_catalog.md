...
| 12 | `pow` / `^`, and `sqrt` | **now matched** (2026-07-31, `docs/69`): both were returning **0** where SR.jl returns **NaN**, so candidates PySR rejects on a non-finite loss were surviving here; `pow` additionally accepted a *near*-integer exponent within 1e-6. `safe_pow` is now a literal transcription of SR.jl's branch table (the obvious simplification is wrong at -Inf) and `safe_sqrt` is `x >= 0 ? sqrt(x) : NaN`, both verified case-by-case against the installed SymbolicRegression.jl. Residual: Julia's `^` evaluates integer-valued `Float64` exponents by repeated squaring, so integer powers differ by 1 ulp — the same class as the libm differences already accepted across platforms | `safe_pow` / `safe_sqrt` in `Operators.jl`; out of domain is NaN, and a NaN cost rejects the candidate |
...
| 15 | Evaluation accounting | **unconditional** per-island counters (2026-07-05, `99cdd3c`, `docs/49`): forward-pass loss evals + LM residual evals summed as `n_evals` in `max_evals` units (`n_evals = forward + lm_resid` by construction); LM Jacobian builds reported but never charged to the budget. Surfaced as `n_evals`/`eval_counts` in the R and Python results. Integer-only, RNG/float-free — the search trajectory is bit-identical with the counters present (Operon-style honest compute accounting for fair budget comparisons) | SR.jl accumulates a scalar `num_evals` internally for its `max_evals` stop; no per-kind breakdown, not returned in the fit result |
| 16 | Duplicate-evaluation cache (`eval_cache`) | **opt-in, default OFF** (2026-07-05, `6d0d683`, `docs/49`): per-island 1024-slot direct-mapped memo of full-data `sse_current`, keyed by a strict FNV-1a 64-bit hash of the postfix tree's evaluation-relevant fields (constant bit patterns and slot indices included) with a field-wise tree-equality guard on every hit, so a collision can only lower the hit rate, never change a value. A hit is charged to `n_evals` exactly like a real evaluation, so results are **bit-identical** ON vs OFF (incl. `max_evals`-budgeted runs); `cache_hits`/`cache_misses` disambiguate real compute. Inactive under batching. Measured (`docs/49`, Stage-1): hit rate 0.19–0.34, total wall −15.8%, median per-problem −9.7%; the default stays OFF per the pre-registered bar (three short runs > +2% slower) | no equivalent — SR.jl re-evaluates every candidate; no loss memoisation |
...
14. **Arbitrary user operators** — rsymbolic2 has a fixed operator enum
    ...
    **Partially addressed** (2026-07-23, `docs/57`) by opt-in **macro operators**:
    ...
    the tree at construction so the node set stays closed. Default empty, so the search
    is bit-identical to the parity default when unused. Arbitrary compiled functions
