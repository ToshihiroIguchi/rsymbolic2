...
| 5 | `warmup_maxsize_by` = 0.0 (off) | PySR *has* a maxsize warmup ramp (`curmaxsize` grows over iterations); rsymbolic2 has **no such mechanism** — maxsize is fixed. Matches only because PySR's default is off |
