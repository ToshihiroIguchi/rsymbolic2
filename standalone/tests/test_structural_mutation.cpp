...
void test_delete_never_grows() {
    SearchSpace space;
    std::mt19937_64 rng(11);
    // ((a*x) + b): the '+' has an operator child, so delete can splice out an operator.
    // Faithful SR.jl delete may instead replace a leaf target with a fresh random leaf
    // (same size), so it never grows but does not always shrink.
    Tree tree = {constant_node(0, 2.0), variable_node(0), binary_node(BinaryOp::Mul),
                 constant_node(1, 3.0), binary_node(BinaryOp::Add)};
    const std::size_t before = tree.size();
    const bool ok = delete_random_op(space, tree, rng);
    CHECK(ok);
    CHECK(tree.size() <= before);  // delete never grows the tree
    CHECK(structurally_valid(tree));
}
