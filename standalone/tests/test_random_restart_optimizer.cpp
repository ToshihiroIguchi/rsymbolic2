...
void test_factory() {
    ...
    // SelfLM is the default least-squares backend and must be constructible.
    auto lm = OptimizerFactory::create(OptimizerType::SelfLM, test_config());
    CHECK(lm != nullptr);
    CHECK(lm->name() == "SelfLMOptimizer");
